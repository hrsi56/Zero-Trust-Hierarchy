...
export default {
  ...
  methodProvenance: {
    verified: [
      'The Builder Assignment form — one Lead-defined piece, an observable piece-level goal with concrete criteria and citations, an exact ownership allowlist, required evidence, and the rule that a Builder must never define, copy in, or pre-answer the independent acceptance oracle that will judge it — is templates/3-builder-assignment.md.',
      'That a Builder never grades its own work or declares a checkpoint criterion complete, and that a Component Critic checks one immutable or uniquely versioned candidate while deriving its acceptance oracle independently rather than adopting a Builder-supplied expected result, is RULEBOOK.md §2 (Tier 3) and §8, "Component Critic protocol," with the forms in templates/4 and templates/5.',
      'That the Integration Critic is a separate fresh context that checks the complete final candidate and the currency of every relied-on component verdict, and cannot close the checkpoint or choose disposition, is RULEBOOK.md §10, "Fresh Integration review," and §2 (Tier 3), with templates/6.',
      'The terminal states (PASS, BLOCKED, PLATEAU, BUDGET_EXHAUSTED — plus the pre-execution BRIEF_INVALID), and that the Engineering Lead returns exactly one of them in one Return Packet and then stops, are RULEBOOK.md §12, "Terminal outcomes, silence, and abandonment," and §13, "Workbench and Return Packet."',
      'The framing of "fresh context" and read-only limits as cooperative procedural controls rather than OS-level or cryptographic isolation is article.md §9 ("These are cooperative controls, not a claim of operating-system isolation") and the bootstrap payload in §14 ("cooperative unless stronger enforcement is separately demonstrated"), and is preserved in spirit in this stage\'s generated prompt.',
    ],
    adapted: [

...
function operatingModeText(fresh) {
  return fresh
    ...
    : 'Continue in the same agent conversation that completed the previous step. That continuity does not exempt this stage from verification — this project\'s own precedence rules hold that actual state always overrides an expected-state narrative, including a narrative from earlier in this very conversation. A document you drafted yourself two turns ago is not evidence that it was ratified, and your own earlier read of it is not evidence that it still says the same thing now.';
}
...
export default {
  ...
  methodProvenance: {
    verified: [
      'The FIT / FIT_WITH_REDUCED_PROFILE / NOT_FIT fit result, its three defining properties (a real versioned artifact, a disagreeable and falsifiable acceptance condition, and meaningful error cost), and the instruction that NOT_FIT means the ceremony costs more than the risk it controls and this method should not be adopted, are drawn directly from the method brief\'s description of the bootstrap\'s intake-and-fit-check phase.',
      'The anti-self-ratification discipline — that a setup agent must never infer ratification from a document\'s existence, from removing a DRAFT label, from its own claim of readiness, from silence, or from mere permission to inspect — is stated directly in the method brief\'s bootstrap section and is preserved here without softening.',
      'The requirement that the bootstrap check every governing artifact for actual ratification and internal consistency before any real checkpoint begins, and that it propose (but not itself issue) the first eligible checkpoint once fit is established, is drawn directly from the method brief\'s description of the bootstrap\'s two-phase structure.',
    ],
    adapted: [],
    productDesign: [
  ...
  advanced: {
    ...
    customization: 'A short terminology note: some of this guide\'s own earlier planning documents, while this stage set was still being drafted, used a placeholder label — "CP-0" — for something like this fit-check-and-bootstrap concept. That term never appears in the actual source method and is not used anywhere in this guide\'s generated prompts, questions, or reports; the only correct names for this mechanic are "the bootstrap" and "the fit check," and any lingering reference to "CP-0" anywhere else should be treated as a stray artifact of early drafting, not a real part of the method.',

...
function operatingModeText(ctx) {
  return ctx.mode === 'same'
    ? 'Continue in the same agent conversation that completed the previous step. Do not treat what that conversation concluded about this project as still true by default: the rulebook rule that actual state overrides expected-state narrative applies to your own earlier turns just as much as to a stale document, so re-verify the current versioning and documentation state before building anything on it.'
    : 'Launch the agent from the root of your project and make sure it can read the project files. Do not copy your project documents into this website — this prompt is meant to be pasted into that agent, and it must read your actual repository itself rather than work from a description of it.';
}
...
export default {
  ...
  methodProvenance: {
    verified: [
      'The precedence order used throughout this stage\'s prompt — ratified root governance, then durable state naming the exact ratified anchor, then the exact plan section containing the current bar, then the execution-role contract, then forms and planning maps, then verified actual state — is the RULEBOOK\'s precedence chain; the brief is explicit that a newer-looking or longer file is never automatically authoritative, only ratification (or verified reality) is.',
      'The rule that the governance-locked set — root governance, ratified anchors, the governance record, role/agent configuration — may not be modified by any agent merely because a checkpoint would benefit is stated directly in the method brief\'s governance section, and shapes this stage\'s constraints and stop conditions.',
      'The six execution/evidence-profile primitives named in this stage\'s prompt (target workspace; immutable/unique candidate identity; independently resolvable evidence identity linked to the candidate; a reviewed-dependency change-detection query; an isolation/single-writer mechanism; a preservation/reclamation procedure) come directly from the checkpoint brief\'s field 7 (GIT_REFERENCE or a DECLARED_EQUIVALENT), generalized here to any versioning substrate.',
    ],
    adapted: [],
    productDesign: [

...
export default {
  ...
  completionGate: [
    ...
    { id: 'artifact-created', label: 'A project-specific Rulebook document was created or revised, naming what each role may inspect, modify, validate, commit, publish, stop, or escalate.', kind: 'confirm', required: true },
    ...
    { id: 'reviewed', label: 'I reviewed the resulting Rulebook myself, and it accurately reflects my destructive-action boundaries and escalation preferences.', kind: 'confirm', required: true },
  ...
  buildLayers(answers, freeText, ctx) {
    ...
    const roleAndAuthority = [
      `You are drafting or revising the Rulebook for ${ctx.projectName} — the governing document that fixes authority boundaries, the conflict-resolution order, and the terminal-outcome vocabulary for every agent that works on this project from here on, including future instances of you.`,
    ...
    const stageObjective = `Produce, or revise, a single project-specific Rulebook for ${ctx.projectName} that adapts a generic authority-and-governance structure to fit this project's actual roles, risk, and constraints — not a copy of a generic template pasted in unchanged, and not an invented structure disconnected from how this project actually works.`;
    ...
    const investigation = ctx.mode === 'fresh'
      ? [
        ...
        '- Identify which of the six method roles this project already treats as distinct people or agents versus combined into one, and any prior plans, decisions, or source-of-truth documents already established that this Rulebook must stay consistent with rather than silently override.',
      ...
      : [
        ...
        '- Whether a Rulebook or equivalent governance document already exists in the project and, if so, exactly what it currently says.',
    ...
    const task = [
      ...
      '',
      '1. Roles actually present in this project. The method defines up to six authority tiers and no others: Architect/Owner (the human, who alone ratifies purpose, governance changes, and disposition), Orchestrator (owns what/when/where and one bounded unit of work\'s exact brief, never prescribes how), Engineering Lead (owns how — architecture, decomposition, the sole version-writer role — never weakens its own bar, enlarges its own scope, publishes, or decides to keep or discard its own work), Builder (implements one assigned piece inside an explicit boundary, never grades its own work), Component Critic (reviews one piece against the bar with fresh eyes, independent of whoever built it), and Integration Critic (a further fresh, independent reviewer of the complete result). Name explicitly which of these exist as distinct roles here versus which are combined or not yet in use — do not invent a role beyond this list, and do not add tiers or ceremony this project\'s actual risk does not justify.'
        + (riskLabel ? ` The human previously described this project\'s overall risk tolerance as ${riskLabel} — calibrate how many of the six roles this Rulebook actually needs to keep distinct accordingly; more separate reviewers or stricter ceremony than that risk justifies is itself a defect in the document, not rigor.` : ''),
      ...
      '',
      'Where the source structure does not fit this project cleanly, adapt it — do not force a six-role hierarchy onto a one-person project, and do not silently drop a boundary that matters here. State explicitly, inside the drafted document, which parts you adapted and why, so the human can see your reasoning rather than just your conclusion.',
    ].filter(Boolean).join('\n');
    ...
    const deliverables = 'A single project-specific Rulebook document, new or revised, inside the project\'s own repository, covering every item in the task above, clearly labeled as a DRAFT pending the human\'s review until they explicitly ratify it. If a governance or rules document already exists, revise it in place rather than creating a duplicate, and call out exactly what changed and why.';

    const qualityGates = [
      'Before returning, verify: every role named in the document is one of the six defined above, or is explicitly marked as combined or not-yet-in-use rather than invented; every destructive-action boundary listed above appears as an explicit, unambiguous prohibition, not a paraphrase that could be read as optional; the precedence order names this project\'s actual documents, not generic placeholders; and the escalation and governance-change instructions are concrete enough that a future agent, reading only this document with no memory of this conversation, could follow them correctly.',
    ].join('\n');
    ...
    const approvalBoundary = 'You may draft or revise the Rulebook document itself, but it remains a proposal until the human reviews it through their own process. You may not treat your own draft as already ratified, may not act as though any rule change you also propose is already in force, and may not perform any of the destructive actions named above under any circumstance in this stage — nor delegate them to yourself, nor ask a different agent to perform them on your behalf without the human\'s separate, explicit approval each time.';
  ...
  advanced: {
    ...
    judgmentVsInvestigation: 'Judgment belongs entirely to the human here: which actions are irreversible enough to be non-negotiable, how eagerly the agent should interrupt with questions, and who drafts the wording of a future rule change are all preference and risk-tolerance calls no amount of investigation can answer. Investigation belongs entirely to the agent: what branches, deploy targets, and credentials actually exist right now; whether a governance document already exists and what it currently says; and which of the six method roles this project already treats as distinct versus combined are all facts the agent must go verify, never facts the structured questions should ask the human to already know.',
    promptAnatomy: 'This stage\'s prompt puts unusual weight on the precedence and constraints layers relative to most stages, because the entire point of a Rulebook is to be the document later prompts point back to when something conflicts — a vague or hedged precedence layer here undermines every later stage\'s authority, not just this one. The task layer deliberately separates "reproduce the human\'s fixed boundaries verbatim" from "adapt the six-role structure to fit this project," because those are two different kinds of instruction: one must never be softened, the other should be actively rewritten to fit.',
    ...
    inputsAndSources: 'Structured inputs are the three questions on this page plus the shared free-text field, which doubles as the description of any "Other" destructive boundary. Investigated inputs are the project\'s actual current branch, deployment, and credential setup, plus any pre-existing governance document. Cross-stage input is the risk tolerance recorded during Orientation, which should calibrate how much of the six-role structure this particular project actually needs.',
    ...
    failureModes: [
      ...
      'The agent forces a full six-role structure onto a small, low-stakes project because it never checked the risk tolerance recorded earlier, producing ceremony nobody will actually follow.',
    ...
    customization: 'If this project is genuinely small and low-stakes, say so in the drafted document rather than performing a six-role hierarchy nobody will use — name which roles are combined into one person or one agent, and why that is an honest match for the actual risk rather than a shortcut taken quietly. If the project is higher-stakes than the four listed boundaries assume, it is equally legitimate to add a stricter boundary in the free-text field — those four are a floor the method names explicitly, not a ceiling on what you are allowed to require.',
